        ...
        if(token != NULL)
        {
            ...
            switch(token[0])
            {
                ...
                case 'D': 
                    ...
                    {
                        if(verbose)
                            printf("DSA page set to on\n");
                        flags[DSA_PAGE] = 1;
                    }
                    else
                    {
                        if(verbose)
                            printf("DSA page set to off\n");
                        flags[DSA_PAGE] = 0;
                    }
                    break;
                case 'O':  /* covert display and output flags */
                    tmp = strtol(++token,&ptr_end,0);
                    if(ptr_end != NULL && tmp > 0)
                    {
                        if(verbose)
                            printf("Flags changed : %i\n",tmp);

                        int bit = 1;
                        flags[OUTPUT_DATA] = bit & tmp ? 1 : 0;
                        bit <<= 1;
                        flags[COUT_DATA_HEX] = bit & tmp ? 1 : 0;
                        bit <<= 1;
                        flags[COUT_DATA_CHAR] = bit & tmp ? 1 : 0;

                        printf("OUTPUT DATA %i\n",flags[OUTPUT_DATA]);
                        printf("COUT_DATA_HEX %i\n",flags[COUT_DATA_HEX]);
                        printf("COUT_DATA_CHAR %i\n",flags[COUT_DATA_CHAR]);
                    }
                    else
                    {
                        if(verbose)
                            printf("Flags set all off\n");

                        flags[OUTPUT_DATA] = 0;
                        flags[COUT_DATA_HEX] = 0;
                        flags[COUT_DATA_CHAR] = 0;
