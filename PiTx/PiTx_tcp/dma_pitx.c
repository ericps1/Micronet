...
void dma_fsk_transmit(int *data, int len)
{
        ...
        /* must be in main call dma_disable if main exits */
        atexit(dma_disable);

//        do
//        {
            //!!!
            //dont rewrite the data just sleep at the end in an infinite 
            //loop and let the DMA stay enabled
            ...
            //for (;;) 
            {
                ...
                int repeat_count = 0;
                while(repeat && repeat_count < NUM_REPEATS)
                {
                    usleep(1000000*len/mod_freq);
                    repeat_count++;
                }
